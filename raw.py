...
class NmapTool:
    def __init__(self, toolDir):
        ...
        self.outputPrompt = "Enter output file name (leave empty for no output): "
        self.nmapLogo = """
        ====================================
        |           Nmap Scanner           |
        ====================================
    ...
    def menu(self, target):
        while True:
            ...
            try:
                if response == "1":
                    self.run_scan(target, ["-sV"], output_file)
                elif response == "2":
                    self.run_scan(target, ["-Pn"], output_file)
                elif response == "3":
                    self.run_scan(target, ["-A"], output_file)
                elif response == "4":
                    ports = input("Enter ports (e.g., 80,443): ")
                    self.run_scan(target, ["-p", ports], output_file)
                elif response == "5":
                    self.run_scan(target, ["-O"], output_file)
                elif response == "6":
                    custom_options = input("Enter custom Nmap options: ")
                    self.run_scan(target, custom_options.split(), output_file)
                elif response == "7":
                    self.save_results(target, output_file)
                elif response == "99":
                    exit(0)
                else:
    ...
    def run_scan(self, target, options, output_file):
    ...
    def save_results(self, target, output_file):
        if output_file:
    ...
    def get_output_file_name(self):
        return input(self.outputPrompt).strip()
